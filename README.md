...
Opinionated template on how to start a Vite + Node.js backend project. Optimized for resource efficiency and security.

## Features

- Full-stack with full BE/FE separation
- Excellent security
- Small Docker production image
- Small package count
...
- Rename the repo with the included init script `node init.js`.
...
This template uses a monorepo setup with pnpm workspaces:

- `packages/backend/`: Node.js + Hono.js API server with Drizzle ORM, migrations, and example todos feature
